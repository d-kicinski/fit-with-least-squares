...
class LSM(object):
  ...
  def predict(self, x, y):
    ...
    y_predict[0:kf-1] = y[0:kf-1]  # put know init values into predicton array
    #print(y)
    #print(y_predict)
    ...
    for k in range(kf, kl):
      ...
      for i in range(1, self.dynamic + 1):
        for j in range(1, self.poly + 1):
          model_oe = np.hstack([model_oe, x[k-i] ** j, y_predict[k-i] ** j])
  ...
  def _calc_M_matrix(self, x, y):
    ...
    for i in range(1, self.dynamic + 1):
      for j in  range(1, self.poly + 1):
        M = np.hstack([M, x[kf-i:kl-i, None] ** j,
                        y[kf-i:kl-i, None] ** j])
